...
// #endif
	...
	if (gateSize("face$o") > (int)sizeof(interface_t) * 8)
	{
		...
		msg << "Node " << getIndex() << " has " << gateSize("face$o") << " output ports. But the maximum "
			<< "number of interfaces manageable by ccnsim is " << sizeof(interface_t) * 8 << " beacause the type of "
...
void core_layer::handle_interest(ccn_interest *int_msg)
{
	...
	{
		std::stringstream msg;
		msg << "I am node " << getIndex() << " and I received an interest from interface " << int_msg->getArrivalGate()->getIndex() << ". This is an error since there is "
		...
		if (llEval && stable)
		{
			if (!(ContentStore->__check_client(int_msg->getArrivalGate()->getIndex())))
			{
				...
				if (getIndex() == 0)
				{
					...
					if (nextNode == 1)
					{
						cout << SIMTIME_DBL(simTime()) << "\t LL-T1 - 1"
							 << "\tContent # " << __id(chunk) - 1 << endl;
					}
				}
				else if (getIndex() == 1)
				{
					...
					if (nextNode == 3)
					{
						cout << SIMTIME_DBL(simTime()) << "\t LL-T2 - 1"
							 << "\tContent # " << __id(chunk) - 1 << endl;
					}
				}
				else if (getIndex() == 3)
				{
					...
					if (nextNode == 7)
					{
						cout << SIMTIME_DBL(simTime()) << "\t LL-T3 - 1"
							 << "\tContent # " << __id(chunk) - 1 << endl;
					}
				}
				// With percentiles
...
void core_layer::handle_decision(bool *decision, ccn_interest *interest)
{
	...
	if (!interest_has_been_forwarded)
	{
		...
		std::stringstream msg;
		msg << "I am node " << getIndex() << " and interest for chunk " << interest->getChunk() << " has not been forwarded. "
			<< ". One of the possible repositories of this chunk is " << interest->get_repos()[0] << " and the target of the interest is " << interest->getTarget() << ". affirmative_decision_for_client = " << affirmative_decision_from_client << ". affirmative_decision_for_arrival_gate = " << affirmative_decision_from_arrival_gate << ". I would have sent the interest to interface " << last_affermative_decision;
		severe_error(__FILE__, __LINE__, msg.str().c_str());
	}
#endif
}
...
{
	if (repo_load != interests - discarded_interests - unsatisfied_interests - interests_satisfied_by_cache)
	{
		std::stringstream msg;
		msg << "node[" << getIndex() << "]: "
			<< "repo_load=" << repo_load << "; interests=" << interests << "; discarded_interests=" << discarded_interests << "; unsatisfied_interests=" << unsatisfied_interests << "; interests_satisfied_by_cache=" << interests_satisfied_by_cache;
	...
	if (!it_has_a_repo_attached && repo_load > 0)
	{
		...
		msg << "node[" << getIndex() << "] has no repo attached. "
			<< "repo_load==" << repo_load << "; repo_interest==" << repo_interest;
		severe_error(__FILE__, line, msg.str().c_str());
	}
	...
	{
		std::stringstream ermsg;
		ermsg << "caches[" << getIndex() << "]->decision_yes=" << ContentStore->get_decision_yes() << "; caches[i]->decision_no=" << ContentStore->get_decision_no() << "; cores[i]->data=" << data << "; cores[i]->repo_load=" << repo_load << "; cores[i]->unsolicited_data=" << unsolicited_data << ". The sum of "
			  << "decision_yes + decision_no + unsolicited_data must be data";
...
void core_layer::add_to_pit(chunk_t chunk, int gateindex)
{
	...
	{
		std::stringstream msg;
		msg << "You are inserting a pit entry related to interface " << gateindex << ". But the number of ports is " << gateSize("face$o");
		severe_error(__FILE__, __LINE__, msg.str().c_str());
	}
	...
	{
		std::stringstream msg;
		msg << "You are inserting a pit entry related to interface " << gateindex << ". But the maximum interface "
	...
	if (bit_op_result > pow(2, gateSize("face$o") - 1))
	{
		...
		std::stringstream ermsg;
		ermsg << "I am node " << getIndex() << ", bit_op_result=" << bit_op_result << " while the number of ports is " << gateSize("face$o") << " and the max number that I should observe is " << pow(2, gateSize("face$o")) - 1;
		ermsg << ". (1<<34)=" << (1 << 34);
		severe_error(__FILE__, __LINE__, ermsg.str().c_str());
	}
...
int core_layer::send_data(ccn_data *msg, const char *gatename, int gateindex, int line_of_the_call)
{
	...
	{
		std::stringstream msg;
		msg << "I am node " << getIndex() << ". Line " << line_of_the_call << " commands you to send a packet to interface " << gateindex << ". But the number of ports is " << gateSize("face$o");
		severe_error(__FILE__, __LINE__, msg.str().c_str());
	}
